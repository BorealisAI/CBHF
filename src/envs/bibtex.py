...
class BibtexEnv(Env):
    ...
    def __init__(self):
        ...
        # checking whether files already exists.

        if not os.path.exists("envs/data/Bibtex_features.npy") or not os.path.exists(
            "envs/data/Bibtex_rewards.npy"
        ):
            self.file_directory = "envs/data/Bibtex_data.txt"
            self.X, self.y = self.parse_data(self.file_directory)
            np.save("envs/data/Bibtex_features.npy", self.X)
            np.save("envs/data/Bibtex_rewards.npy", self.y)

        else:
            self.X = np.load("envs/data/Bibtex_features.npy")
            self.y = np.load("envs/data/Bibtex_rewards.npy")
