...
class DeliciousEnv(Env):

    id = "delicious-v0"

    def __init__(self):

        if not os.path.exists("envs/data/Delicious_features.npy") or not os.path.exists(
            "envs/data/Delicious_rewards.npy"
        ):
            self.file_directory = "envs/data/Delicious_data.txt"
            self.X, self.y = self.parse_data(self.file_directory)

            np.save("envs/data/Delicious_features.npy", self.X)
            np.save("envs/data/Delicious_rewards.npy", self.y)

        else:

            self.X = np.load("envs/data/Delicious_features.npy")
            self.y = np.load("envs/data/Delicious_rewards.npy")
